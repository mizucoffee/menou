...
register_test('file_exists') do |option, test_tb, path|
...
register_test('file_hash') do |option, test_tb, path|
  option['files'].each_with_index do |file, idx|
    test_tb.task(file) do |error|
      ...
      error.call "\"#{file}\"のハッシュ値が合致しません。結果: #{digest} 期待する値: #{option['expect'][idx]}" unless option['expect'][idx] == digest
    end
  end
end
